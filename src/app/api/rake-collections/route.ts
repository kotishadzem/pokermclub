...
export async function POST(req: NextRequest) {
  ...
  const rakeCollection = await prisma.rakeCollection.create({
    data: {
      tableId,
      amount,
      notes: notes || null,
  ...
  bumpVersion();
  return NextResponse.json(rakeCollection, { status: 201 });
}
...
export async function GET(req: NextRequest) {
  ...
  const collections = await prisma.rakeCollection.findMany({

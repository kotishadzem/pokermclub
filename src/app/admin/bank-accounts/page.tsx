...
export default function BankAccountsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    try {
      const url = editItem ? `/api/bank-accounts/${editItem.id}` : "/api/bank-accounts";
      ...
      setToast({ message: editItem ? "Bank account updated" : "Bank account created", type: "success" });
      setModalOpen(false);
      fetchItems();
    } catch {
  ...
  async function handleDelete() {
    ...
    await fetch(`/api/bank-accounts/${confirmDelete.id}`, { method: "DELETE" });
    setConfirmDelete(null);
    setToast({ message: "Bank account deactivated", type: "success" });
    fetchItems();
  }
  ...
  return (
    <div style={{ animation: "floatUp 0.5s ease-out forwards" }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-wide" style={{ fontFamily: "var(--font-display)" }}>Bank Accounts</h1>
          <p className="mt-1 text-sm text-muted">Configure available bank accounts</p>
        </div>
        <button
          onClick={openCreate}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={2} className="px-5 py-8 text-center text-muted">No bank accounts</td></tr>
            ) : (
              items.map((item) => (
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onMouseDown={(e) => { if (e.target === e.currentTarget) setConfirmDelete(null); }}>
          <div className="w-full max-w-sm rounded-xl border border-card-border bg-card-bg p-6 shadow-2xl" style={{ animation: "floatUp 0.2s ease-out" }}>
            <h3 className="text-lg font-bold mb-2" style={{ fontFamily: "var(--font-display)", color: "var(--danger)" }}>Delete Bank Account</h3>
            <p className="text-sm text-muted mb-5">This will deactivate <strong className="text-foreground">{confirmDelete.name}</strong>.</p>
